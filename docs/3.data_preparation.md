...
### Docker for Synthesis (gen_synth.py)

- **pyproject.toml**, **Dockerfile**, **run_synth.bat** in synthesis/: Создаёт изолированное окружение с uv и albumentations (решает проблемы сборки на Windows).
  - Build: `docker build -t tbank-synth -f data_preparation/synthesis/Dockerfile data_preparation/synthesis`
  - Run: `data_preparation/synthesis/run_synth.bat 10` (для N=10 теста, монтирует весь проект).
- **Note:** gen_synth.py fallback to PIL if albumentations not installed (no advanced aug like GaussNoise/MotionBlur). Для локального запуска: `cd data_preparation/synthesis && uv sync && python gen_synth.py N=10`.
- Требует Docker Desktop. Образ tbank-synth можно публиковать для быстрого использования без локальной сборки.
