...
- Note: Требует Docker Desktop. gen_synth.py использует albumentations для augmentations. Local run: `cd data_preparation/synthesis && uv sync && python gen_synth.py --N 2000`.
...
Docker-образ для генерации синтетических данных логотипов Т-Банка. Включает gen_synth.py с albumentations augmentations, uv для deps (albumentations, pillow, numpy). Используйте docker run с mounts для /app/synthesis и /app/data, arg --N для количества изображений. Пример: docker run -v $(pwd)/data_preparation/synthesis:/app/synthesis -v $(pwd)/data:/app/data --rm medphisiker/tbank-synth python gen_synth.py --N 2000. Сгенерированные данные сохраняются в data/data_synt/. Поддержка 3 классов логотипов.
