...
- **pyproject.toml**, **Dockerfile**, **docker-build-run.bat** in synthesis/: Создаёт изолированное окружение с uv. uv sync installs deps, no albumentations issue.
  ...
  - Manual (run from data_preparation/synthesis/):
    ...
    - Run (Windows, in cmd): `docker run -v "%cd%:/app/synthesis" -v "%cd%/../../data:/app/data" --rm tbank-synth python gen_synth.py --N 10` (test small, mounts synthesis/data). In PowerShell: `docker run -v "${PWD}:/app/synthesis" -v "${PWD}/../../data:/app/data" --rm tbank-synth python gen_synth.py --N 10`.
