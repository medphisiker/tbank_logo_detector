...
- **crop_logos.py** (в synthesis/): Кропит логотипы из официальных референсов по аннотациям COCO; сохраняет кропы в /data_preparation/synthesis/crops/.

- **gen_synth.py** (в synthesis/): Генерация синтетических изображений с наложением логотипов на фоны (synthesis generation).

- **download_backgrounds.py** (в synthesis/): Скачивает фоны из Unsplash; **требует VPN для доступа к Unsplash**.

### Docker for Synth Gen

- **pyproject.toml**, **Dockerfile**, **docker-build-run.bat** in synthesis/: Создаёт изолированное окружение с uv. uv sync installs deps, no albumentations issue.

  - Run from data_preparation/synthesis/: `docker-build-run.bat` (builds with context synthesis, runs test N=10).

  - Manual (run from data_preparation/synthesis/):
    - Рекомендуется: Pull готового образа: `docker pull medphisiker/tbank-synth:latest`
    - Run с готовым образом (Windows, в cmd): `docker run -v "%cd%:/app/synthesis" -v "%cd%/../../data:/app/data" --rm medphisiker/tbank-synth python gen_synth.py --N 10` (test small, mounts synthesis/data). In PowerShell: `docker run -v "${PWD}:/app/synthesis" -v "${PWD}/../../data:/app/data" --rm medphisiker/tbank-synth python gen_synth.py --N 10`.
    - Run с готовым образом (Linux/macOS): `docker run -v "$(pwd):/app/synthesis" -v "$(pwd)/../../data:/app/data" --rm medphisiker/tbank-synth python gen_synth.py --N 10` (test small, mounts synthesis/data).
    - Альтернатива: Соберите свой образ.
      - Build: `docker build -t tbank-synth -f Dockerfile .` (context synthesis for local pyproject.toml)
      - Run (Windows, в cmd): `docker run -v "%cd%:/app/synthesis" -v "%cd%/../../data:/app/data" --rm tbank-synth python gen_synth.py --N 10`. In PowerShell: `docker run -v "${PWD}:/app/synthesis" -v "${PWD}/../../data:/app/data" --rm tbank-synth python gen_synth.py --N 10`.
      - Run (Linux/macOS): `docker run -v "$(pwd):/app/synthesis" -v "$(pwd)/../../data:/app/data" --rm tbank-synth python gen_synth.py --N 10`.

    - Run (Linux/macOS): `docker run -v "$(pwd):/app/synthesis" -v "$(pwd)/../../data:/app/data" --rm tbank-synth python gen_synth.py --N 10` (test small, mounts synthesis/data).

- Note: Requires Docker Desktop. gen_synth.py fallback to PIL if albumentations not installed. Local: `cd data_preparation/synthesis && uv sync && python gen_synth.py --N 10`.

For publish: `docker login; docker tag tbank-synth yourusername/tbank-synth:latest; docker push yourusername/tbank-synth:latest`.
...
- Pull: `docker pull medphisiker/tbank-synth:latest`

- Пример запуска (из корня проекта): `docker run -v $(pwd)/data_preparation/synthesis:/app/synthesis -v $(pwd)/data:/app/data --rm medphisiker/tbank-synth python gen_synth.py --N 2000`

  Для Windows (PowerShell): `docker run -v "${PWD}/data_preparation/synthesis:/app/synthesis" -v "${PWD}/data:/app/data" --rm medphisiker/tbank-synth python gen_synth.py --N 2000`

[Готовый образ на https://hub.docker.com/r/medphisiker/tbank-synth](https://hub.docker.com/r/medphisiker/tbank-synth) (описание: "Docker image for generating synthetic T-Bank logo dataset using gen_synth.py with UV deps, PIL fallback for aug").

Примечание: Чтобы добавить описание на Docker Hub, отредактируйте в UI после push или используйте Docker CLI, если поддерживается.

### Разработки

Организованы скрипты предобработки в data_preparation/ с поддиректориями synthesis/, yoloe/, colab_prep/, label_studio/; независимые ZIP для поддиректорий; автоматическая исправление путей в COCO аннотациях.
