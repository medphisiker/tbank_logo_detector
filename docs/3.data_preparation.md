...
- **gen_synth.py** (в synthesis/): Генерация синтетических изображений с наложением логотипов (3 класса: purple=0, white=1, yellow=2) на фоны. Поддержка аргумента --N для количества изображений (default 20). PIL fallback для augmentations (brightness/contrast) если albumentations недоступен. Вывод: data/data_synt/images/labels/{train/val/test} в YOLO формате. Пример: `python gen_synth.py --N 2000`.
...
- **pyproject.toml**, **Dockerfile**, **docker-build-run.bat/sh**, **run_synth.bat** в synthesis/: Создаёт изолированное окружение на базе python:3.10-slim с uv для deps (albumentations, pillow, numpy). uv lock && uv sync --frozen, активирует .venv. Копирует synthesis/ в /app/synthesis. Mounts: /app/synthesis (crops/backgrounds) и /app/data (output data_synt). CMD: python gen_synth.py (поддержка --N или env N).
  ...
  - **run_synth.bat** (из корня): Build tbank-synth из synthesis/, run с mount всего проекта (/workspace), env N (default 20). Пример: `run_synth.bat 2000`.

  - Manual build (из data_preparation/synthesis/): `docker build -t tbank-synth -f Dockerfile .`

  - Run с mount (из корня проекта): `docker run -v $(pwd)/data_preparation/synthesis:/app/synthesis -v $(pwd)/data:/app/data --rm tbank-synth python gen_synth.py --N 2000` (Linux/macOS). Для Windows: `-v "%CD%/data_preparation/synthesis:/app/synthesis" -v "%CD%/data:/app/data"`. Альтернатива env: `-e N=2000`.

- Note: Требует Docker Desktop. gen_synth.py использует PIL fallback для augmentations если albumentations недоступен. Local run: `cd data_preparation/synthesis && uv sync && python gen_synth.py --N 2000`.

Для publish: `docker login; docker tag tbank-synth medphisiker/tbank-synth:latest; docker push medphisiker/tbank-synth:latest`.
...
- Пример запуска (из корня проекта): `docker run -v $(pwd)/data_preparation/synthesis:/app/synthesis -v $(pwd)/data:/app/data --rm medphisiker/tbank-synth python gen_synth.py --N 2000`
  ...
  Для Windows (cmd): `docker run -v "%CD%/data_preparation/synthesis:/app/synthesis" -v "%CD%/data:/app/data" --rm medphisiker/tbank-synth python gen_synth.py --N 2000`. PowerShell: `docker run -v "${PWD}/data_preparation/synthesis:/app/synthesis" -v "${PWD}/data:/app/data" --rm medphisiker/tbank-synth python gen_synth.py --N 2000`.

[Готовый образ на https://hub.docker.com/r/medphisiker/tbank-synth](https://hub.docker.com/r/medphisiker/tbank-synth).

Примечание: Сгенерированные данные сохраняются в data/data_synt/. Поддержка 3 классов логотипов.

### Docker Hub Description

Docker-образ для генерации синтетических данных логотипов Т-Банка. Включает gen_synth.py с PIL augmentations (fallback без albumentations), uv для deps (ultralytics, pillow etc.). Используйте docker run с mounts для /app/synthesis и /app/data, arg --N для количества изображений. Пример: docker run -v $(pwd)/data_preparation/synthesis:/app/synthesis -v $(pwd)/data:/app/data --rm medphisiker/tbank-synth python gen_synth.py --N 2000. Сгенерированные данные сохраняются в data/data_synt/. Поддержка 3 классов логотипов.

Категория: Machine Learning or Data Processing.

[https://hub.docker.com/r/medphisiker/tbank-synth](https://hub.docker.com/r/medphisiker/tbank-synth)
