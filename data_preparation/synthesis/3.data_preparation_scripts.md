...
Генератор создает синтетические изображения с наложением логотипов Т-Банка (3 класса: purple=0, white=1, yellow=2) на случайные фоновые изображения. Поддерживает аугментации (brightness/contrast, noise, blur, HSV) и генерирует данные в формате YOLO для обучения моделей детекции.

Вывод: `data/data_synt/images/labels/{train/val/test}` с автоматическим разделением на train/val/test (80/10/10).

Подробности см. в [`data_preparation/synthesis/synthesis_generator/README.md`](synthesis_generator/README.md).
...
**Назначение**: Удобный скрипт для последовательного запуска всего пайплайна синтеза данных.

**Функциональность**:
- Принимает параметр `N` для количества изображений (default: 20)
- Последовательно выполняет:
  1. Обрезку логотипов (`crop_logos.py`)
  2. Скачивание фонов (`download_backgrounds.py`)
  3. Генерацию синтетики (`gen_synth.py --N <N>`)
- Использует Docker-контейнер `tbank-synth` или `medphisiker/tbank-synth:latest`
...
- Автоматическая нормализация путей для Docker
...
**Назначение**: Сборка и запуск Docker-контейнера для генерации синтетики.

**Функциональность**:
- Собирает образ `tbank-synth` из `Dockerfile`
- Монтирует весь проект в контейнер (`/workspace`)
- Передает переменную окружения `N` (default: 20)
- Запускает `gen_synth.py` в контейнере
...
**Особенности**:
- Полная сборка образа при каждом запуске
- Mount всего проекта для доступа ко всем данным

### docker-build-run.bat

**Расположение**: `data_preparation/synthesis/docker-build-run.bat`

**Назначение**: Скачивание готового образа и тестовая генерация.

**Функциональность**:
- Скачивает образ `medphisiker/tbank-synth:latest`
- Запускает тестовую генерацию с `N=10`
- Монтирует `synthesis/` и `data/` директории
...
**Особенности**:
- Использует готовый опубликованный образ
...
**Назначение**: Аналог `run_synthesis_pipeline.bat` для Unix-систем.

**Функциональность**:
- Аналогична Windows-версии
- Использует `$PWD` для текущей директории
...
Docker-образ `tbank-synth` предоставляет изолированное окружение для генерации синтетических данных. Основан на `python:3.10-slim` с использованием uv для управления зависимостями.

**Включает**:
- Python 3.10
- uv package manager
- Зависимости: albumentations, pillow, numpy, tqdm, requests
- Пакет `synthesis_generator`
- Все скрипты из `data_preparation/synthesis/`
...
```cmd
cd data_preparation/synthesis
run_synth.bat  # Соберет и запустит
```
...
```cmd
docker run -v "%CD%/data_preparation/synthesis:/app/synthesis" -v "%CD%/data:/app/data" --rm tbank-synth -e N=2000 python gen_synth.py
```

### Готовый образ

Рекомендуется использовать опубликованный образ без локальной сборки:
...
Для быстрого запуска всего пайплайна синтеза данных:
