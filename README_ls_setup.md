...
- Исправит пути к изображениям для корректной работы в Docker контейнере
- Покажет инструкции по импорту в Label Studio
- Обработает возможные ошибки

**Преимущества нашего скрипта:**
- Автоматически исправляет пути к изображениям (оригинальный конвертер этого не делает)
- Использует `uv` для управления зависимостями
- Обеспечивает корректную работу в Docker контейнере
- Все шаги автоматизированы

### Вариант 2: Использование Label Studio Converter (Альтернатива)
```bash
...
**Важно:** Оригинальный конвертер не исправляет пути к изображениям, поэтому после конвертации может потребоваться ручная корректировка путей.
